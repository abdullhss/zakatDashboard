...
  // === الصلاحيات ===
  // نعتمد على privType لتحديد الصلاحيات التي يتم جلبها (M للإدارة، O للمكتب)
  const privType: "M" | "O" =
...
  // قيمة افتراضية / تصفير الصلاحية بناءً على نوع الحساب
  useEffect(() => {
    // 💡 التعديل: إذا كان نوع الحساب مكتب، نجعل الصلاحية 0 ونخرج.
    if (UserType === "O") {
        setGroupRightId(0); // 0 يعني "لا صلاحية من الإدارة العامة"
        return;
    }

    // إذا لم يكن هناك قيمة للصلاحية وحمّلت الصلاحيات المتاحة للإدارة (M)
    if (UserType === "M" && !privLoading && privileges.length && !GroupRight_Id) {
        const firstId = Number(privIdOf(privileges[0])) || 0;
        if (firstId) {
            setGroupRightId(firstId);
        }
    }
...
    // 💡 التعديل: الصلاحية (GroupRight_Id) مطلوبة فقط عندما يكون UserType="M" (إدارة)
...
    // نتحقق من الصلاحية فقط إذا كان النوع 'M'
...
          // نرسل الصلاحية المختارة (ستكون 0 إذا كان UserType="O")
...
                    // 💡 تعطيل الحقل إذا كان نوع المستخدم "O" (مكتب)
                    isDisabled={
                        privLoading || 
                        privileges.length === 0 || 
                        UserType === "O" // معطل لحسابات المكاتب
                    } 
                  >
                    {
                        // عرض خيارات الصلاحية فقط إذا كان UserType ليس "O" أو لم يتم تحميلها
                        UserType !== "O" ? (
                            ...
                            // عرض خيار صوري مع القيمة 0 عندما يكون مكتب
                            <option value="0" disabled={UserType === "O"}>
                                تُحدد داخليًا (مُرسلة بـ 0)
                            </option>
                        )
                    }
                  </FieldSelect>
