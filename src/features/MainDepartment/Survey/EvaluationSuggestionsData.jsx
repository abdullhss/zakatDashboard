...
const PROC_KEY = "nOQOEKwVuC+VoeOE0r0jJM+n1fIUuStxfdmfFHTXxBo="
...
const EvaluationSuggestionsData = () => {
  ...
  const docTitle =
    'الخدمات المضافة: (الخدمات المضافة: ما هي الميزات أو الخدمات التي تقترح إدراجها في المنصة مستقبلاً؟)'

  const preparePrint = useCallback(async () => {
    if (totalRows <= 0) return
    const response = await executeProcedure(PROC_KEY, `1#${totalRows}`)
    const raw = response.decrypted.data?.Result[0].EvaluationSuggestionsData
